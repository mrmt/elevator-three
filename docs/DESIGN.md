...
- 履歴 `hist`: シーン、フレーズ、イベント、ダブ、リフ (音名とリズムの型)、キー。種類ごとに直近8件を持つ。
  送るときは配列ではなく1本の英文 `story` に畳む (D-21。D-20 の小節窓を置き換えた)
...
### D-19. Jev の判断を DECISION セクションに分ける (2026-09-19)

D-17 で質問が増えて1件の JSON が長くなり、D-15 の強調の行 (音楽的に何が起こるか) がスクロールしないと見えなくなった。
右カラムの JEV の下に DECISION セクションを足し、そこへ移した。

- 高さは3行に固定する。1件を1行にし、はみ出す分は省略記号で切る。全文と、決めた小節・問い合わせの名前はマウスを乗せると出る (`title`)
- 新しい順に40件まで持ち、あふれた分は中でスクロールする。いちばん新しい件だけを太字と `--hot` の帯で強調する
- DECISION に出すのは Jev が決めたことだけにする。`…: decided by local rules` と `discarded (…)` は、どの問い合わせの話かが要るので JEV の件に残す
- JEV は残りの高さを取るので、DECISION の分だけ縮む。monitor と DECISION の位置と大きさは、やり取りが増えても変わらない

### D-20. 履歴は直近16小節ぶんだけ送る (2026-09-21) — D-21 で置き換えた

`hist` は種類ごとに直近8件を持つが、シーンやキーのように間隔の長いものは何分も前の出来事が残る。
古い出来事はいまの判断にほとんど効かず、Jev への入力トークンを無駄に使うだけなので、
`jevState()` で `bar >= barCount-16` の件だけに絞って送ることにした (`recentHist()`)。

実測 (bar 102) では履歴が 1108字 → 259字に減ったが、代わりに失うものが大きかった。
リフは40小節あく回があり、窓の中が0件になると `riff.riff` の「最近のリフに答える」前提が消える。
シーンは1つが32〜96小節なので窓には常に現在の1件しか入らず、`scene.next` の「最近聴いた雰囲気を避ける」も効かない。

### D-21. 履歴はナラティブにして渡す (2026-09-21)

配列を窓で切る代わりに、`narrateHistory()` が履歴を1本の英文 `story` に畳んで渡す。
古い出来事を捨てるのではなく、直近は具体的に、それ以前は件数だけにして畳む。

- 節は固定 (経過 → シーン → キー → フレーズ → イベント → リフ → ダブ)。各節に入れる具体例は2〜4件までなので、
  長く鳴らしても文の長さが増えない。実測で 174〜508字 (全件 JSON は 1108字、伸び続けると約1600字)
- 小節は絶対値ではなく「何小節前か」で語る。いまの小節は `state.bar` が別に持っている
- 8件からあふれた分は `histTotal` の件数として残る。「12 parallel shifts」「10 ornament events so far」のように、
  セット全体の積み上がりは数で伝わる
- 例: `3 min in, 111 bars. 2 scenes so far, lately dusk (jazz & bossa) → frost (stab / dark), 47 bars in of a planned 80.
  Keys have walked G# minor → F minor → D# minor → B minor; 12 parallel shifts, the last 6 bars ago. …`
- 数値の `history` は送らない。同じ事実を2通りで渡すと Jev が読み分ける必要が出るため
- 1文になったので、JEV セクションの表示から外す必要もなくなった (D-16 の除外を解除)
- 最後に平行移動した小節は `lastShiftBar` として履歴とは別に持つ。8件の枠は転調なしのキー履歴で溢れるので、
  枠から探すと「N 回転調した」と言いながら「何小節前か」が引けず、`jevState()` が例外で落ちる。
  `jevAsk()` は `jev.inflight` を立てたあとに `jevState()` を呼ぶので、落ちるとその種類の問い合わせが二度と出なくなる
- リフは「累計 N 回」と「いまのモチーフが手元の履歴に何回あるか」を分けて言う。
  `histTotal.riffs` はモチーフを問わない累計なので、これを「このモチーフを N 回」と言うと嘘になる
